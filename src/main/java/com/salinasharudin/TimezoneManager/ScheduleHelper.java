...
public final class ScheduleHelper {
	// Map will store all the hours that all members have mutually available for
	// each day
	...
	public static void createMutualSchedule(ArrayList<Contact> others) {
		...
		// Terminate if there was an error obtaining the int representation of the
		// offset
	...
	// Move coordinates of day,time to where it matches up with the other zone using
	// the dif
	...
	static int[] convertTime(int dif, int day, int hr) {
		...
		if (new_hr < 0) {
			...
			// If previous day was already the first day of the week, set it to the last day
			// of the week
			if (new_day - 1 < 0) {
				new_day = 6;
			} else {
	...
	public static Boolean checkTime(Contact other, int day, int hr) {
		if (other.getAvailability()[day][hr] != null && other.getAvailability()[day][hr] == true)
			return true;
		else
			return false;
	}
	...
	// Gets the difference in offset between the user and the other contact. For
	// example, -05 (user) and +08 (other) have a +13 difference.
