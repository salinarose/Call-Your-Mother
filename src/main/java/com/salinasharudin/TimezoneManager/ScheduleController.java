...
public class ScheduleController implements Initializable {
	...
	public void updateList() {
		...
		// Check the number of items in the list and enable the calculate button if it
		// has between 1-3 items
		int items = listSelected.getItems().size();
		if (items > 0) {
	...
	public void selectTime() {
		...
		for (Node node : gridResults.getChildren()) {
			...
			node.setOnMouseExited(e -> {
				// Only changes the background color back to default if it isn't currently
				// selected
				// Right now it stays highlighted if you press the add button, then click a
				// different time
	...
	public void addToSchedule() {
		...
		// Loop through all the results, looking for one that contains a day in the
		// label until we have reached the row of the selected node
		for (Node node : gridResults.getChildren()) {
		...
		// If key already exists, ask "A call on DAY at TIME with CONTACTS already
		// exists. Would you like to overwrite with PEOPLE? "
		String message;
		if (FileHelper.getCalls().containsKey(day + time)) {
			message = "A call on " + day + time + " with " + FileHelper.getCalls().get(day + time)
					+ " already exists. Would you like to overwrite with" + people.toString() + "?";
		} else {
